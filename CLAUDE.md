...
Required: `DATABASE_URL`, `AUTH_SECRET`, `RESEND_API_KEY`, `AUTH_EMAIL_FROM`, `FOOTBALL_DATA_TOKEN` (also powers the per-minute live-score sync at `/api/live-scores`; the free tier lags in-play matches by a few minutes — API-Football would be fresher but its free plan can't serve the current season), `CRON_SECRET`.
